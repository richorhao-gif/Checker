...
describe('Hero chrome', () => {
  it('renders the English channel badge through the hero locale seat', () => {
    const view = render(<HeroShell t={makeTranslate(en, commonEn)} />)
    expect(view.getByText('Reviewed with evidence, sealed with confidence')).toBeTruthy()
    expect(view.getByText('Catering Channel · Bid Review')).toBeTruthy()
  })
})
...
describe('ConversationRoot resident composer', () => {
  ...
  it('hero phase: same textarea, hero chrome, no header, picker switches the workspace', () => {
    ...
    expect(b.view.getByText('审之有据 · 落印为凭')).toBeTruthy()
    expect(b.view.getByText('餐饮渠道 · 标书审核')).toBeTruthy()
  ...
  it('settling phase: a summary that does not prove the session blank hides the composer while it opens', () => {
    ...
    expect(b.view.queryByText('审之有据 · 落印为凭')).toBeNull()
  })
