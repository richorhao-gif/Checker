...
The `/client` exports are the plugin body (`apply`/`inject`), the `BidReviewComposer`, `QualificationsEditor`, and `ReviewDesk` components, the fixed question, the output contract, and `buildBidReviewPrompt`, the desk derivations (`deriveDesk` plus the copy decisions that read a stage, verdict, wait, or basis into a dictionary key or a seal ink), and the injected face, surface, desk, outcome, and document types. Interface copy lives in the `bidReview` namespace dictionaries in both languages.
...
One ordinary user message, composed by this package and submitted through the standard input actions. [`src/client/prompt.ts`](src/client/prompt.ts) owns the literal `BID_REVIEW_PRESET_QUESTION` and the literal `BID_REVIEW_VERDICT_CONTRACT` that requires the declaration line the seal reads; the fields follow them in a fixed order, with the uploaded document's absolute server path and then the shared qualifications text verbatim. The Agent reads the document itself from that path with its own file tools. No tool, prompt section, or Session event is registered by this package, and the model never sees the composer's local steps.
...
The question and the output contract are fixed copy of 310 characters (926 UTF-8 bytes), so their cost is the same on every conversation and every deployment. The qualifications text is the only growing part and is bounded by the Host's `maxQualificationsBytes` (64 KiB in the Web bundle); an empty record contributes its heading and one empty line. The document path adds one line, and the document's own content is never inlined by this package.

#### KV Cache effect

The prompt is a user message at the head of a fresh conversation, so it forms that conversation's prefix. Two conversations sharing the same qualifications text reuse the question, the contract, and that text; a save between them changes the tail of the prefix and therefore the cache entry from the qualifications onward. Nothing this package does touches an in-flight conversation's history.
...
- **The fixed question and the output contract are not localizable** — both are literal product copy rather than dictionary entries, because they are submitted to the model as a user message and localizing them would change what every deployment's Agent receives. Changing either is a source edit in `prompt.ts`.
...
- **The opinion is the model's closing text** — the sheet's body is the last assistant message carrying text, so a review that ends on a tool call renders the empty-report copy, and a sheet whose head carries no declaration line wears an uncommitted seal, which is where every review recorded before the contract existed now lands. Neither is corrected here, because a verdict the model did not state would make the sheet lie; a real seal on an old review means submitting it again.
- **The declaration is read in the sheet's head only** — `verdictOf` searches the first eight lines, so a sheet that commits further down wears an uncommitted seal even though the model did commit. The contract asks for the first line and the eight are the allowance for a title and a blockquote above it; reading the whole body instead is what let a quoted requirement or a conditional seal a verdict the sheet never gave.
